...
public class ClassInfo extends CommonInfo {
    ...
    public int nonStaticFields() {
        ...
        if (getSuperClassIndex() < classLoader.lowestNativeId)
            sum += getClassLoader().getClassInfo(getSuperClassIndex()).nonStaticFields();

        return sum;
    }

    public int getSuperClassIndex() {
        int index = getClassLoader().getClassIndex(getSuperClassName());

        // no such class in the local classes, try native
        if (index < 0)
            index = classLoader.getNativeClassId(getSuperClassName());
